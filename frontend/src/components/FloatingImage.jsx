...
export default function FloatingImage({ image, onUpdate, onDelete, isEditable, isSelected, selectedText, onSelectText, onSelect, viewMode = 'desktop' }) {
  ...
  const isMobile = viewMode === 'mobile' || (!isEditable && isMobileWindow);

  // Active X percent position
  const activeXPercent = isMobile 
    ? (image.mobileXPercent !== undefined ? image.mobileXPercent : (image.xPercent || 0))
    : (image.xPercent || 0);

  // Active Y px position
  const activeY = isMobile
    ? (image.mobileY !== undefined ? image.mobileY : (image.y || 0))
    : (image.y || 0);

  // Active width percent
  const activeWidthPercent = isMobile
    ? (image.mobileWidthPercent !== undefined ? image.mobileWidthPercent : (image.widthPercent || 40))
    : (image.widthPercent || 20);

  // Active height px
  const activeHeight = isMobile
    ? (image.mobileHeight !== undefined ? image.mobileHeight : (image.height || 150))
    : (image.height || 150);
  ...
  const handleMouseDownMove = (e) => {
    ...
    const containerWidth = container ? container.clientWidth : (isMobile ? 390 : 1000);

    startPos.current = { x: e.clientX, y: e.clientY };
    const currentX = (activeXPercent / 100) * containerWidth;
    startDim.current = { x: currentX, y: activeY, containerWidth };
  };
  ...
  const handleMouseDownResize = (e, direction) => {
    ...
    startPos.current = { x: e.clientX, y: e.clientY };
    const currentX = (activeXPercent / 100) * containerWidth;
    const currentW = (activeWidthPercent / 100) * containerWidth;
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (isDragging) {
        ...
        const boundedXPercent = Math.min(100 - activeWidthPercent, xPercent);
        const boundedY = Math.max(0, y + dy);
        
        if (isMobile) {
          onUpdate({
            ...image,
            mobileXPercent: boundedXPercent,
            mobileY: boundedY
          });
        } else {
          onUpdate({
        ...
        if (isResizing.includes('e')) w = Math.max(30, w + dx);
        if (isResizing.includes('s')) h = Math.max(30, h + dy);
        ...
        const calculatedXPercent = (x / containerWidth) * 100;
        const calculatedWidthPercent = (w / containerWidth) * 100;

        if (isMobile) {
          onUpdate({
            ...image,
            mobileXPercent: calculatedXPercent,
            mobileY: y,
            mobileWidthPercent: calculatedWidthPercent,
            mobileHeight: h
          });
        } else {
          onUpdate({
            ...image,
            xPercent: calculatedXPercent,
            y,
  ...
  }, [isDragging, isResizing, image, onUpdate, isMobile, activeWidthPercent]);
